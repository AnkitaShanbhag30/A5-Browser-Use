(function () {
  ...
  function injectPanel() {
    ...
    style.textContent = `
		...
		/* Minimal styling for the doc link field (optional) */
		.advanced-settings-doclink {
		  margin-top: 10px;
		  width: 100%;
		  padding: 8px;
		  font-size: 14px;
    ...
    panel.innerHTML = `
		...
		<div class="command-content">
		  ...
		  <div class="advanced-settings-container" id="advancedSettingsContainer">
			...
			<!-- Field to directly add a doc link (PDF, Google Docs, etc.) -->
			<input
			  type="text"
			  class="advanced-settings-doclink"
			  id="advancedSettingsDocLink"
			  placeholder="Enter Google Docs or Drive (PDF) link..."
			/>
    ...
    // Field for optionally entering doc link
    const advancedSettingsDocLink = shadowRoot.getElementById("advancedSettingsDocLink");
    ...
    //    (Now sending doc link to POST /run, where the backend handles fetching)
    // ----------------------------------------
    runButton.addEventListener("click", async () => {
      ...
      try {
        ...
        chrome.storage.local.get(["agenticAdvancedContext"], async (result) => {
          ...
          // Check if user entered a doc link in the advanced settings
          const docLink = advancedSettingsDocLink.value.trim();

          // Build request body
          const requestBody = { task: taskWithContext };
          if (docLink) {
            // If there's a doc link, pass it in 'url'
            requestBody.url = docLink;
          }
